  ...
  return (
    <AdminShell active="pricing" title="단가표 관리">
      ...
      <section className="mt-10">
        <h2 className="text-[14px] font-bold text-ink">현재 적용 중인 단가</h2>

        <h3 className="mt-5 text-[13px] font-bold text-ink">내지 ({paper.length}행)</h3>
        <PricingTable
        ...
        <h3 className="mt-6 text-[13px] font-bold text-ink">제본 ({binding.length}행)</h3>
        <PricingTable
        ...
        <h3 className="mt-6 text-[13px] font-bold text-ink">박스 ({box.length}행)</h3>
        <PricingTable
        ...
        <p className="mt-6 text-[11px] text-ink-sub">
...
function PricingTable({
  ...
  if (rows.length === 0) {
    return (
      <p className="mt-2 rounded border border-line bg-bg px-3 py-6 text-center text-[12px] text-ink-sub">
        데이터가 없습니다.
      </p>
    );
  }
  return (
    <div className="mt-2 overflow-x-auto rounded border border-line">
      <table className="w-full border-collapse text-[12px]">
        <thead className="bg-bg text-left text-[11px] text-ink-sub">
          <tr>
            {headers.map((h, i) => (
          ...
          {rows.map((r, i) => (
            <tr key={i} className="border-t border-line">
              {r.map((cell, j) => (
                <td
                  key={j}
                  className={`px-3 py-1.5 ${j === r.length - 1 ? "text-right font-medium text-ink" : "text-ink-sub"}`}
                >
                  {cell}
                </td>
              ))}
