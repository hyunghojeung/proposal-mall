    ...
    <>
      ...
      <main className="mx-auto min-h-[60vh] max-w-page px-6 py-10">
        ...
        <div className="flex items-baseline justify-between border-y border-line py-4">
          <h1 className="text-[24px] font-black tracking-tight text-ink">{activeLabel}</h1>
        ...
        <ul className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {products.length === 0 ? (
            <li className="col-span-full py-16 text-center text-[16px] text-ink-sub">
            ...
            products.map((p) => (
              <li key={p.id}>
                <Link
                  ...
                  className="group block h-full rounded border border-line bg-white p-7 transition-all hover:-translate-y-0.5 hover:border-brand hover:shadow-[0_8px_24px_rgba(0,0,0,.06)]"
                >
