...
export function CheckoutForm({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  const [transferComplete, setTransferComplete] = useState<TransferComplete | null>(null);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (payment.adapter === "transfer") {
        ...
        setTransferComplete({
          serial:       orderSerial,
          total:        data.order?.totalAmount ?? total,
          productNames: data.order?.productNames ?? items.map((it) => it.productName),
  ...
  if (hydrated && items.length === 0 && !transferComplete) {
    return (
      ...
      <div className="mx-auto max-w-lg py-10 text-center">
        ...
        <div className="mt-6 overflow-hidden rounded-lg border border-line bg-white text-left">
          ...
          <div className="divide-y divide-line text-[14px]">
            {[
              { label: "상품",     value: productLabel },
              { label: "결제 금액", value: `${orderTotal.toLocaleString()}원` },
